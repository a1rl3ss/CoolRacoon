...
st.set_page_config(page_title="Raccoon AI iPad Edition", layout="wide")
st.title("🦝 Raccoon AI System")
st.write("Нажми START ниже, чтобы запустить камеру")
...
# Инициализация MediaPipe (вне класса для скорости)
...
# Класс-обработчик видео для iPad
class RaccoonTransformer(VideoTransformerBase):
    ...
    def transform(self, frame):
        ...
        if pose_res.pose_landmarks:
            ...
            elif (l_wrist.z < l_shoulder.z - 0.7) or (r_wrist.z < r_shoulder.z - 0.7):
                emotion = "gun.png"
        ...
        return np.hstack((img, raccoon))

# Запуск стрима
webrtc_streamer(key="raccoon-filter", video_transformer_factory=RaccoonTransformer)
